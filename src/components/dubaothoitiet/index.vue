    ...
    <div class="weather-predict-container">
            ...
            <form @submit.prevent="predictWeather" class="prediction-form">
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input v-model="weatherData.precipitation" type="number" step="0.1" min="0" required
                            class="form-control" />
                    </div>

                    <div class="form-group">
                        <label><i class="fas fa-temperature-high"></i> Nhiệt độ cao nhất
                            (°C)</label>
                        <input v-model="weatherData.temp_max" type="number" step="0.1" required class="form-control" />
                    </div>
                </div>

                <div class="form-row">
                    <div class="form-group">
                        <label><i class="fas fa-temperature-low"></i> Nhiệt độ thấp nhất
                            (°C)</label>
                        <input v-model="weatherData.temp_min" type="number" step="0.1" required class="form-control" />
                    </div>

                    <div class="form-group">
                        <label><i class="fas fa-wind"></i> Tốc độ gió (km/h)</label>
                        <input v-model="weatherData.wind" type="number" step="0.1" min="0" required
                            class="form-control" />
                    </div>
                </div>
        ...
        <div class="weather-info-card">
            ...
            <div class="info-content">
                <p>
                    Nhập các thông số thời tiết vào form để nhận dự đoán thời tiết chính
                    xác.
                </p>
                <ul>
                    <li><strong>Lượng mưa:</strong> Lượng mưa dự kiến (mm)</li>
                    <li>
                        <strong>Nhiệt độ cao nhất:</strong> Nhiệt độ cao nhất trong ngày
                        (°C)
                    </li>
                    <li>
                        <strong>Nhiệt độ thấp nhất:</strong> Nhiệt độ thấp nhất trong ngày
                        (°C)
                    </li>
    ...
    methods: {
        async predictWeather() {
            try {
                ...
                if (error.code === "ECONNREFUSED") {
                    this.error =
                        "Không thể kết nối đến máy chủ. Vui lòng kiểm tra xem máy chủ đã được khởi động chưa.";
                } else if (error.response && error.response.data) {
                    this.error = error.response.data.message;
                } else {
                    this.error =
                        "Có lỗi xảy ra khi thực hiện dự đoán. Vui lòng thử lại sau.";
                }
            }
        },
        ...
        getWeatherIcon() {
            ...
            if (predictionLower.includes("nắng") || predictionLower.includes("sun")) {
                ...
                predictionLower.includes("tuyết") ||
                predictionLower.includes("snow")
            ) {
                return "/images/weather/snow.png";
            } else if (
                predictionLower.includes("mưa lớn") ||
                predictionLower.includes("rain")
            ) {
                return "/images/weather/rain.png";
            } else if (
                predictionLower.includes("mưa phùn") ||
                predictionLower.includes("drizzle")
            ) {
                return "/images/weather/drizzle.png";
            } else if (
                predictionLower.includes("sương mù") ||
                predictionLower.includes("fog")
            ) {
        ...
        getWeatherDescription() {
            ...
            if (predictionLower.includes("nắng")) {
                description =
                    "Thời tiết nắng đẹp, thích hợp cho các hoạt động ngoài trời.";
